...
the provider-selection and prompt-guard logic run for real, while the run step
is replaced with a fake result so mapping and error handling are verified in
isolation.
"""
...
def test_generate_images_maps_and_returns(monkeypatch):
    ...
    class FakePipe:
        def run(self, *, sink=None, timeout=None):
            return fake_result
...
def test_generate_images_raises_on_failed_step(monkeypatch):
    fake_result = types.SimpleNamespace(
        run=types.SimpleNamespace(steps=[]),
        manifest=types.SimpleNamespace(manifest_uri=None),
